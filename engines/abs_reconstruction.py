...
class DistributedReconstruction:
    ...
    def train(self):
        ...
        for i in tqdm(range(int(cfg_train['max_epoch']))):
            ...
            if (i + 1) % 10 == 0:
                ...
                if self.args.local_rank == 0:
                    test_result = json.load(open(test_result_path, "r"))["loss"]
                    self.logger.info("test loss: " + str(test_result))
    ...
    def test(self, train_epoch=None):
        ...
        with open(test_filepath, "w") as f:
            json.dump({"loss": {key: float(test_loss[key]) for key in test_loss}}, f)
