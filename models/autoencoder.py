...
class EncoderAE1D(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            cout = base_channels * (2**i)
            cin = in_channels if i == 0 else cout // 2
            self.layers.append(EncoderBlock1D(cin, cout, 2, 2))
...
class DecoderAE1D(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            cin = base_channels * (2 ** (num_layers - i - 1))
            cout = in_channels if i == num_layers - 1 else cin // 2
            self.layers.append(DecoderBlock1D(cin, cout, 2, 2))
...
class FrameAutoEncoder1D(nn.Module):
    def __init__(
        self,
        in_channels: int = 1,
        base_channels: int = 8,
        ...
        """
        super().__init__()
        self.encoder = EncoderAE1D(in_channels, base_channels, num_layers)
        self.decoder = DecoderAE1D(in_channels, base_channels, num_layers)
    ...
    def encode(self, x):
        # x is expected to be a tensor of shape [batch, num_sources, frames, w, h].
        # Convert it to [batch * num_sources * frames, w, h]
        batch, num_sources, frames, w = (
            x.shape[0],
            x.shape[1],
            x.shape[2],
            x.shape[3]
        )
        x = x.contiguous().view(batch * num_sources * frames, 1, w)
    ...
    def reconstruct(self, x):
        ...
        # Undo the view of x. y has [batch * num_sources * frames, w, h]
        # Convert it to [batch, num_sources, frames, w, h]
        y = y.view(batch, num_sources, frames, w)
        return y