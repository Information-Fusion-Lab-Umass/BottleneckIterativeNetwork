...
from models.autoencoder import FrameAutoEncoder, FrameAutoEncoder1D
from models.afrcnn import AFRCNN, GlobLN
    ...
    def forward(
        ...
        for i in range(self.num_blocks):
            ...
            if return_intermediate:
                intermediate.append(Ri.detach().cpu().numpy())
        if return_intermediate:
            return intermediate
...
class AVLITFixAE(nn.Module):
    ...
    def forward(self, x: torch.Tensor, v: torch.Tensor, return_intermediate=False):
        # Get sizes of inputs
        b, T = x.shape[0], x.shape[-1]
        M, F = v.shape[1], v.shape[2]
        ...
        # Get video features, fv
        # fv = self.video_encoder.encode(v)
        fv = v.permute(0, 1, 3, 2).reshape(b, self.num_sources * self.video_embedding_dim, -1)
        # fv = v.permute(0, 1, 3, 2).reshape(b, M * self.video_embedding_dim, -1)
        fv = self.video_bottleneck(fv)

        # Forward the video and audio branches
        # fv_p = self.video_branch(fv, return_intermediate)
        # fa_p = self.audio_branch(fa, fv_p, return_intermediate)
        #
        # if return_intermediate:
        #     return {"audio": fa_p, "video": fv_p}
        fv_p = self.video_branch(fv)
        fa_p = self.audio_branch(fa, fv_p)
