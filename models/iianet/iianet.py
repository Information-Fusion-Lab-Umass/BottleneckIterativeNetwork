...
import math
from models.iianet.base_av_model import BaseAVModel
...
class AudioTopDown(nn.Module):
    ...
    def forward(self, residual, output, global_f, multi_vs=None):
        ...
        for idx in range(self.depth):
            if multi_vs != None:
                tmp_a = self.loc_glo_fus[idx](output[idx],
                                              F.interpolate(global_f, size=output[idx].shape[-1], mode="nearest"))
                tmp = tmp_a + self.act(F.interpolate(multi_vs[idx], size=output[idx].shape[-1], mode="nearest")) * tmp_a
            else:
                tmp = self.loc_glo_fus[idx](output[idx],
                                            F.interpolate(global_f, size=output[idx].shape[-1], mode="nearest"))
            x_fused.append(tmp)
...
class Recurrent(nn.Module):
    def __init__(self, out_channels=128, in_channels=512, vin_channels=64, vout_channels=64, upsampling_depth=4,
                 _iter=4):
        super().__init__()
        self.iter = _iter
    ...
    def forward(self, x, v):
        ...
        for i in range(self.iter):
            if i in [0]:
                ...
                global_a, global_v = self.InterA_T(torch.stack(output_mlp_a, dim=1).sum(1),
                                                   torch.stack(output_mlp_v, dim=1).sum(1))
...
class IIANet(BaseAVModel):
    def __init__(
            ...
            out_channels=128,
            in_channels=512,
            vpre_channels=1024,  # prev default 512
            vin_channels=64,
            vout_channels=64,
            num_blocks=16,
            upsampling_depth=5,
        ...
        # Separation module
        self.pre_v = nn.Conv1d(vpre_channels * num_sources, vin_channels, kernel_size=3, padding=1)
    ...
    def forward(self, input_wav, mouth_emb):
        # input shape: (B, T)
        b, v_pre_dim = mouth_emb.shape[0], mouth_emb.shape[3]
        was_one_d = False
        if input_wav.ndim == 1:
            was_one_d = True
        ...
        if mouth_emb.ndim == 4:
            # if mouth embed has multiple sources
            mouth_emb = mouth_emb.permute(0, 1, 3, 2).reshape(b, self.num_sources * v_pre_dim, -1)
