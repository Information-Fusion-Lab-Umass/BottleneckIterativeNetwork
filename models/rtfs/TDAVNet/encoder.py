...
class STFTEncoder(BaseEncoder):
    ...
    def forward(self, x: torch.Tensor):
        ...
        spec = torch.stack([spec.real, spec.imag], 1).transpose(2, 3).contiguous()  # B, 2, T, F  (F is N)
        spec_feature_map = self.conv(spec)  # B, C, T, F

        return spec_feature_map


class STFTSimpleEncoder(BaseEncoder):
    def __init__(
        self,
        win: int,
        hop_length: int,
    ...
    def forward(self, x: torch.Tensor):
        ...
        spec = torch.stack([spec.real, spec.imag], 1)  # B, 2, F, T  (F is N)

        B, _, _, T = spec.shape

        # spec_feature_map = self.conv(spec)  # B, C, T, F
        flatten_spec = spec.reshape(B, -1, T)
        return flatten_spec  # (B, 2*F, T)
