...
class STFTSimpleDecoder(BaseDecoder):
    def __init__(
        self,
        win: int,
        hop_length: int,
    ...
    def forward(self, x: torch.Tensor, input_shape: torch.Size):
        ...
        batch_size, length = input_shape[0], input_shape[-1]
        # print(x.shape)
        x = x.view(batch_size * self.n_src, 2, -1, x.shape[-1])  # B, n_src, 2, T, F -> # B * n_src, 2, F, T
        # decoded_separated_audio = self.decoder(x)  # B * n_src, N, T, F - > B * n_src, 2, T, F

        spec = torch.complex(x[:, 0], x[:, 1])  # B*n_src, F, T
        # spec = torch.stack([spec.real, spec.imag], dim=-1)  # B*n_src, F, T
        # spec = spec.transpose(1, 2).contiguous()  # B*n_src, F, T
