    ...
    def get_MACs(self):
        with torch.no_grad():
            ...
            if self.video_bn_params.get("is2d", False):
                ...
                # video_input = torch.rand(batch_size, seconds * 25,  2 * v_chan).to(device)
            else:
                video_input = torch.rand(batch_size, v_chan * self.n_src, seconds * 25).to(device)
                # video_input = torch.rand(batch_size, self.n_src, seconds * 25, v_chan)

            # print(audio_input.shape, video_input.shape)
