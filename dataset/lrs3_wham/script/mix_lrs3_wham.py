...
LOGGER = distributed.init_logger(is_distributed=False)


def get_audio_feat(self, filepath, sampling_rate, start_pad: int = 0, total_length: int = None):
    signal, _ = audio.process_audio(filepath, target_sr=sampling_rate)
    if total_length is not None:
        signal = signal[:(total_length - start_pad)]
        signal = np.pad(signal, (start_pad, total_length - len(signal) - start_pad), constant_values=0.0)
    return signal


def audio_mixture(split="train", max_duration_ms=2000, sampling_rate=16000):
    audio_max_samples = int(max_duration_ms / 1000 * sampling_rate)
    LOGGER.info(f"Reading mixture file")
    mix_speaker_list = []

    lrs3_wham_meta = pd.read_csv(
        os.path.join(LRS3_ROOT_PATH, "meta", f"mix_2_spk_{split}_fully_overlapped_only_full_wham.csv"))

    for idx, row in lrs3_wham_meta.iterrows():
        mixture_filename = row["mixture_dir"]
        main_speaker_name, main_file_id, background_speaker_name, background_file_id = mixture_filename.split("/")[
            -1].replace(".wav", "").split("_")

        speaker_1 = row["speaker_1_dir"]
        speaker_1_db = row["speaker_1_db"]
        s1_start = row["s1_start"]
        s2_start = row["s2_start"]
        overlap_ms = row["overlap_ms"]
        s1_silence_start = row["s1_silence_start"]
        s1_silence_end = row["s1_silence_end"]
        ...
        audio_mixture = get_audio_feat(mixture_filename,
                                       start_pad=0,
                                       total_length=audio_max_samples
                                       )

        main_speaker_start_audio = int(s1_start / 1000 * sampling_rate)

        clean_audio = get_audio_feat(speaker_1,
